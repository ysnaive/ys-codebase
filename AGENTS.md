...
1. **第一反射原則**：凡以「理解、閱讀、探索」為目的，強制以 `python yscb.py knowledge-db search -s` 為第一反射，杜絕盲目使用原生文字搜尋 (Text Grep) 或檔案閱讀 (File Reader) 工具。
...
3. **新詞主動補足鐵律**：在分析、修改或排查途中，凡遭遇當前上下文未曾具備之任何新名詞、新欄位、新協議或未知概念，嚴禁憑字面臆測，必須即刻將其轉化為語意化查詢（`python yscb.py knowledge-db search '<新詞 業務情境>' -s`），主動補足對應知識上下文後方可繼續推進。
