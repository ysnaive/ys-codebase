...
| **sub_02** | `sub_02_multilingual_tokenizer_and_hybrid_search` | Full Track | `Pending` | **多語言分詞與 BM25+向量複合檢索**：實作中英混雜分詞器；引入輕量 ONNX 多語言 Embedding (`fastembed`)；實作 RRF 融合演算法與平滑降級；徹底移除手刻同義詞庫 (`thesaurus.py`)。 |
...
- [ ] **里程碑 2 (檢索躍升)**：完成多語言 Tokenizer 與 BM25 + 向量 RRF 複合式檢索，打破跨語言檢索壁壘 (sub_02)
