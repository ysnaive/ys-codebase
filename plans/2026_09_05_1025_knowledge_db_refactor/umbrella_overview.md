...
| **sub_05** | `sub_05_pipeline_engine_refactor_and_dogfooding` | Full Track | `Pending` | **流水線解耦與生態系驗收**：拆解 1,800 行 Monolithic `engine.py` 為 Pipeline 架構；核驗對外 CLI 契約；全套件純化測試 100% 回歸；完成 Dogfooding 閉環與發布。 |
...
- [ ] **里程碑 5 (架構收斂)**：完成 Pipeline 引擎解耦、全系統單元測試回歸、Dogfooding 驗證與正式打包發布 (sub_05)
