# 評測任務提示詞：傳統工具對照組 (Agent B)

> **使用說明**：請開立一個全新的 Agent Session，將本提示詞全文作為初次 Prompt 輸入。Agent B 將僅使用傳統搜尋工具（grep/find/view_file）完成基準評測。
...
最終請將所有答案、查詢日誌與指標統計彙整寫入至 `/workspace/ys-codebase/benchmark/results_traditional.md`。
...
完成所有題目後，使用 `write_to_file` 將完整結果寫入 `/workspace/ys-codebase/benchmark/results_traditional.md`，結構如下：
