# 評測任務提示詞：Knowledge-DB 啟用組 (Agent A)

> **使用說明**：請開立一個全新的 Agent Session，將本提示詞全文作為初次 Prompt 輸入。Agent A 將使用新版 `knowledge-db` 工具鏈完成基準評測。

---

## 🎯 任務目標

你是一名高階架構評測專員。當前任務是針對 `/workspace/ys-codebase/benchmark/QUESTIONS.md` 中的 9 道基準題目進行實機探索與解答，並詳實記錄每道題目在檢索過程中的各項效能指標。

最終請將所有答案、查詢日誌與指標統計彙整寫入至 `/workspace/ys-codebase/benchmark/results_knowledge_db.md`。
...
6. **答案內容**：依據題目要求，給出包含檔案路徑、符號名稱、行號與架構邏輯的完整答案。

---

## 📝 產出格式規範

完成所有題目後，使用 `write_to_file` 將完整結果寫入 `/workspace/ys-codebase/benchmark/results_knowledge_db.md`，結構如下：
...
請立刻讀取 `/workspace/ys-codebase/benchmark/QUESTIONS.md` 並開始執行評測任務！
