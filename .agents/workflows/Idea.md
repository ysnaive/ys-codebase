...
本 Workflow 獨立於標準開發 SOP 之外，用於記錄突發奇想、潛在技術改進、未來架構藍圖或輕量原型探索。
...
1. **獨立於 SOP 之外**：
   - 靈感池存放於 `plans://ideas/`，不需要走 Phase 1~7 流程，也不需立即分流判定。
2. **開放式自由對話**：
   - 開發者怎麼問，Agent 怎麼答。靈活探索可能性、架構思路與潛在邊界，不拘泥於死板格式。
3. **提案產出 (What / Why / How / Related)**：
   - 討論收斂後，依 `workflows/templates/idea.md` 模板產出結構化構想書，命名為 `YYYY_MM_DD_{idea_name}.md`。
4. **立項流轉機制 (Promoted to Dev Plan)**：
   - 當靈感成熟並獲指示正式開立 Dev Plan 時，**直接將 Idea 檔案搬移至新開立的 Dev Plan 目錄並轉換為 `P00_semantic_requirements.md`**，達成完美無縫銜接。
...
- 當討論告一段落時，Agent 依 `workflows/templates/idea.md` 建立檔案：
  - **路徑**：`plans://ideas/YYYY_MM_DD_{idea_name}.md`
