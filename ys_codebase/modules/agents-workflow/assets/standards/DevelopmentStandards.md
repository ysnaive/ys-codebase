...
### 4.6 開發歷程自檢與合規評測工作流 (/Retro)
- **觸發時機**：於任何 Session 開發、除錯、問答或計畫結案前後由開發者或 Agent 調用。
- **核心目標**：以當前對話歷史與 Transcript 為對象，回顧開發過程、稽核紀律合規性（三大公理、Turn 限制、問答 $\neq$ 推進、確定性讀檔等）、執行模組宣告自檢項，並產出工作流改進建議。
- **不合規文檔溯源分析剛性紀律**：若發現任何不合規條目，強制執行 5-Whys 根因溯源，明確指出「是因閱讀了哪一份實體文檔或指示導致做出此錯誤決策」，直接暴露文檔缺陷。
- **異常過濾呈遞**：核心紀律採全量檢核但僅呈報不符合項目；若 100% 全數合規僅輸出一行確認。
