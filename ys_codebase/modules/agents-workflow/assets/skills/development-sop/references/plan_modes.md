...
本手冊詳細定義專案 6 大計畫分支的判定標準、生命週期流程、升降級守門與產出物規範。

---

## 1. 標準開發計畫 (Full Track - Level 1)
...
- **剛性要求**：嚴禁跳過任何 Phase（除獲授權執行 `/Auto` 連續推進外）；產出各階段文檔後必須向開發者呈報摘要並立即 End Turn 等待確認。

---

## 2. 迅捷開發計畫 (Fast Track - Level 0)
...
- **生命週期**：
  ...
  - **FT-3 (結案階段)**：確認測試 100% 通過，追加 `CHANGELOG.md`，結案交付。
- 🚨 **強制升級機制**：實作中若代碼超標（$> 100$ 行）或意外觸碰 Public API 變更，強制中斷並升級為 Full Track！

---

## 3. 分類型主計畫 (Umbrella - Level 2)
...
- **實體位置**：`__${workflow.roadmap://}__`（實體路徑 `./plans/roadmap/roadmap.md`）。
- **流轉機制**：
  - 平時持續沉澱與維護技術想法。
  - 開發者調用 `/Roadmap` 時，Agent 智慧探索當前代碼脈絡並推薦最適合當前專案情境之主題。
