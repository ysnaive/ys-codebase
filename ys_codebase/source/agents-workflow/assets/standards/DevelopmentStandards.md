...
- **Phase 4 (定稿審查 P04)**：預排 7 維度文檔衝擊（主動盤點模組手冊、專題手冊與決策手冊）、完成架構靈魂拷問，剛性定稿 P04 與 P06 (Confirmed)。
- **Phase 5 (任務實作 P05)**：依 P04 拓撲順序實作，同步落實微觀代碼註解、型別契約與文檔交付；偏差處置：🚨 **Critical** 退回修正、⚠️ **Major** 暫停回報、ℹ️ **Minor** 紀錄於偏差表。
- **Phase 6 (測試驗證 P06)**：實機執行測試並回填；**人工/UX 驗證 Checkpoint**：明確等待開發者手動驗收或指示免測後方可標記 Passed。
- **Phase 7 (成果展示 P07)**：產出結案報告、落實三層文檔 (宏觀/中觀/微觀) 1:1 交付驗收、追加 `CHANGELOG.md`，實機執行 `plan verify` 合規檢核；計畫目錄預設留存原位。
