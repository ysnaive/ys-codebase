...
5. Review 與 FT-3 (品質審查與結案)：FT-2 通過後強制執行 SOP Review 矩陣核驗（代碼清理、1:1 知識庫與 CHANGELOG 同步），實機調用 `python __${yscb.host://yscb.py}__ agents-workflow plan verify <plan_name>` 驗證計畫完整合規 ➔ Checkpoint 等待結案確認 (Completed)。
...
- [ ] **SOP Review 品質矩陣**：三層文檔對齊（`docs/` 手冊、`DESIGN_NOTES.md`、微觀註解）、測試 100% 通過。
- [ ] **日誌與發布交付**：追加 `project://CHANGELOG.md` 發布摘要。
- [ ] **結構與註解檢核**：實機執行 `python __${yscb.host://yscb.py}__ agents-workflow plan verify <plan_name>` 驗證 100% Passed。
- **結案狀態**：`Completed`
