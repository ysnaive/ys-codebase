"""
Unit, Edge Case, Regression and Performance Tests for Knowledge-DB Call Graph and Reference Index (Level 1 Plan).
"""
...
class TestCallGraphAndReferenceIndex(YSCBTestCase):
    """
    調用圖譜與引用拓撲完整測試套件 (FT-01~07, ET-01~02, PT-01)
    """
...
"""
    ...
    def test_ft_06_call_graph_binary_cache_and_incremental_patch(self):
        """FT-06: 驗證 CallGraphIndex Protocol 5 Gzip 快取持久化與 patch_incremental 差量修補 (FR-05, EC-03)"""
    ...
    def test_ft_07_engine_call_graph_queries_and_cli(self):
        """FT-07: 驗證 KnowledgeEngine 端到端 callers/callees/impact 查詢與 RFC 8089 輸出格式 (FR-06, NFR-04)"""
        with tempfile.TemporaryDirectory() as temp_dir:
...
""",
    ...
    def test_et_01_dynamic_calls_and_error_handling(self):
        """ET-01: 邊界測試 - 動態屬性調用、語法錯誤與無 Import 多型方法安全降級 (EC-01, EC-04)"""
        parser = PythonParser()
...
"""
    ...
    def test_et_02_space_isolation_for_same_symbol(self):
        """ET-02: 邊界測試 - 跨空間同名符號 Tier 3 空間優先隔離綁定 (EC-05)"""
    ...
    def test_pt_01_call_graph_performance_and_memory(self):
        """PT-01: 效能測試 - 500 個節點與 2000 條調用邊在 Gzip 下體積 < 150KB，查詢延遲 < 10ms (NFR-02, NFR-03)"""
        with tempfile.TemporaryDirectory() as temp_dir:
